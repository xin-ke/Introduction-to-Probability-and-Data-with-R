...
sfo_feb_flights %>%
  summarise(median = median(arr_delay), iqr = IQR(arr_delay), range = range(arr_delay))
...
nycflights <- nycflights %>%
  mutate(air_time_hr = air_time/60, avg_speed = distance/air_time_hr)
nycflights %>% 
...
nycflights %>% 
  ...
  summarise(ot_dep_arr = sum(arr_type == "on time") / n())

nycflights %>% 
  filter(dep_type == "delayed") %>%
  summarise(dl_dep_ot_arr = sum(arr_type == "on time") / n())
